...
## How to Play
Each row of the visual grid corresponds to a different musical scale – play the scale by pressing keys from the matching keyboard row, or by touching the pads directly. When the chords change, adjust the scale you're playing on by moving all your fingers to the indicated row at the right time. The number row sounds good at any time!
...
## Architecture
- React + Vite - component-based UI layer
- Vanilla JavaScript classes - connecting app with Web Audio API, MediaRecorder API, rAF loop
...
- Expand in-app music theory guidance
